...
export class RepositoryBase<T extends Document> implements IRead<T>, IWrite<T> {
  ...
  createOnlyOne(item: any, callback: (error: Error, result: any) => void){
     if(item.user instanceof Object){
      const query = {
        "user.user._id":item.user.user._id
      }
      
       this.findOnquery(query,callback);
    }else{
  ...
  create(item: T, callback: (error: any, result: any) => void) {
    console.log(item,"++++++++++++++++",this._model.create);
